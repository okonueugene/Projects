...
class Product_model extends CI_Model{
      ...
      function get_purchase_list(){
        $q = $this->db->query("Select * from plans");
            return $q->result();
            
      }
      function get_purchase_by_id($id){
        $q = $this->db->query("Select * from plans 
            where 1 and id = '".$id."' limit 1");
            return $q->row();
      }
      ...
      function get_sale_order_by_id($order_id){
            $q = $this->db->query("Select *,cast(quantity as decimal(10,3))as quantity from sales_items,products            where sales_items.product_id=products.id AND order_id =".$order_id."");
            return $q->result();
      } 
      ...
      function get_sales($filter=""){
         $sql = "Select  sales.*,cast(quantity as decimal(10,2))as quantity from sales 
      ...
      function get_attendance($filter=""){
        ...
        function get_sup($filter=""){
         $sql = "Select * from sup_transactions 
            where 1 ".$filter." ORDER BY id DESC";
            $q = $this->db->query($sql);
            return $q->result();
      }
      
       function get_advance($filter=""){
