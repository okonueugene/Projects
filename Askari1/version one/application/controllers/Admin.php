  ...
  public function sendmail()
  {
    if (isset($_POST['submit'])) {
      ...
      if ($email == 1) {
        ...
        $body = "<a href=https://askari.optitech.co.ke/admin/modify_password/" . $uid . "'>Click here for reset your password</a>";
  ...
  function add_guards()
  {
    ...
    if (isset($_POST['submit'])) {
      ...
      if ($this->form_validation->run() == FALSE) {
        ...
        $uploaded_image1 =base_url()."img/".$unique_image;
        
        // image file directory
        $target = "img/".basename($uploaded_image1);
        
        
        
        $data = array('name' => $name, 'mobile' => $mobile, 'agency' => $agency, 'password' => $password, 'email' => $email, 'status' => 1, 'start' => $_POST['start'], 'end' => $_POST['end'], 'guard_id' => $_POST['guard_id'],'profilepic' => $uploaded_image1);
        $this->load->model('Common_model');
        $d = $this->Common_model->add('guards', $data, $mobile);
        
        //upload to folder
        $load = move_uploaded_file($_FILES['image']['tmp_name'], $target);
  ...
  function terms()
  {
    ...
    if (isset($_POST['submit'])) {
      ...
      if ($this->form_validation->run() == FALSE) {
        ...
        $this->load->model('Common_model');
        $d = $this->Common_model->edit('terms', $data, 1);
    ...
    $id = $this->uri->segment('3');
    $data['inv'] = $this->db->query('select * from terms where id=1')->row();
...
function privacy_policy()
  {
    ...
    if (isset($_POST['submit'])) {
      ...
      if ($this->form_validation->run() == FALSE) {
        ...
        $data = array('description' => $_POST['description']);
        $this->load->model('Common_model');
        $d = $this->Common_model->edit('policy', $data, 1);
    ...
    $id = $this->uri->segment('3');
    $data['inv'] = $this->db->query('select * from policy where id=1')->row();
...
function policy()
{
   ...
   function accesscode()
  {
    ...
    if (isset($_POST['submit'])) {
      ...
      if ($this->form_validation->run() == FALSE) {
        ...
        $data = array( 'code' => $_POST['description']);
        $this->load->model('Common_model');
        $d = $this->Common_model->edit('access', $data, 1);
    ...
    if (isset($_POST['finish'])) {
      ...
      if ($this->form_validation->run() == FALSE) {
        if ($this->form_validation->error_string() != "")
          $this->session->set_flashdata("message2", '<div class="alert alert-warning alert-dismissible" role="alert">
                                        <i class="fa fa-warning"></i>
                                      <button type="button" class="close" data-dismiss="alert"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button>
                                      <strong>Warning!</strong> ' . $this->form_validation->error_string() . '
                                    </div>');
        ...
        $data = array( 'code' => $_POST['description']);
        $this->load->model('Common_model');
        $d = $this->Common_model->edit('bookoff', $data, 1);

        $this->session->set_flashdata("message2", '<div class="alert alert-success alert-dismissible" role="alert">
                    <i class="fa fa-check"></i>
                  <button type="button" class="close" data-dismiss="alert"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button>
                  <strong></strong>' . $d["msg"] . '
                </div>');
    ...
    $id = $this->uri->segment('3');
    $data['inv'] = $this->db->query('select * from access where id=1')->row();
    $data['in'] = $this->db->query('select * from bookoff where id=1')->row();
    // $this->load->view('admin/accesscode', $data);
  ...
  function edit_otp()
  {
    ...
    if ($this->uri->segment('3')) {
      $id = $this->uri->segment('3');
      $data['inv'] = $this->db->query('select * from guards where id=' . $id)->row();
      $this->load->view('admin/edit_otp', $data);
    }

    if (isset($_POST['submit'])) {
      $this->load->library('form_validation');
      $this->form_validation->set_rules('accesscode', 'Description', 'trim');
      $this->form_validation->set_rules('bookoff', 'Description', 'trim');
      ...
      if ($this->form_validation->run() == FALSE) {
        ...
        $agency = $_POST['agency'];
        //$password=md5($_POST['password']);
        $data = array('accesscode' => $_POST['accesscode'], 'bookoff' => $_POST['bookoff'], 'guard_id' => $_POST['guard_id']);
        $this->load->model('Common_model');
        $d = $this->Common_model->edit('guards', $data, $id);
...
function edit_attendance()
  {
    ...
    if ($this->uri->segment('3')) {
      $id = $this->uri->segment('3');
      $data['inv'] = $this->db->query('select * from attendance where id=' . $id)->row();
      $this->load->view('admin/edit_attendance', $data);
    }
    
    if (isset($_POST['submit'])) {
        $data = array('intime' => date('Y-m-d H:i:s', strtotime($this->input->post('intime'))));
        $this->load->model('Common_model');
        $d = $this->Common_model->edit('attendance', $data, $id);
    ...
    if (isset($_POST['outtime'])) {
        $data = array('outtime' => date('Y-m-d H:i:s', strtotime($this->input->post('outtime'))));
        $this->load->model('Common_model');
        $d = $this->Common_model->edit('attendance', $data, $id);
