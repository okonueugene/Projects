...
/* ========== Categories =========== */
    ...
    public function listcategories1()
    {
       if(_is_user_login($this)){
           $data["error"] = "";
           $data["active"] = "listcat";
           $this->load->model("category_model");
           $data["allcat"] = $this->category_model->get_categories();
           $this->load->view('admin/categories/listcat1',$data);
        }
        else
        {
            redirect('admin');
...
function investors(){
    ...
    $this->db->order_by('activate','asc');
        $data["products"]  = $this->db->get('investors')->result();
...
/* ========== Purchase END ==========*/
    ...
    function registers(){
        ...
        if(_is_user_login($this)){
            ...
            {
                $this->load->library('form_validation');
                $this->form_validation->set_rules('mdate', 'maturity date', 'trim|required');
                $this->form_validation->set_rules('mamount', 'maturity amount', 'trim|required');
                $this->form_validation->set_rules('inv_amount', 'invested amount', 'trim|required');
                ...
                $data=array('date'=>$date,'ch_m_date'=>date('Y-m-d',strtotime($mdate)),'city'=>$city,'selected_plan'=>$this->db->get_where('plans',array('id'=>$plans))->row()->plan_name,'plan_id'=>$plans,'amount'=>$inamount,'m_amount'=>$mamount,'ref_by'=>1,'name'=>$name,'activate'=>0,'mobile'=>$mobile);

                 if($_FILES["proof_img"]["size"] > 0){
                        $config['upload_path']          = './uploads/proofs/';
                        $config['allowed_types']        = 'gif|jpg|png|jpeg';
                        $this->load->library('upload', $config);
                        ...
                        {
                            $img_data = $this->upload->data();
                            $data["proof_img"]=$img_data['file_name'];
                        }
            ...
            $sms="http://bulksms2.wkbusiness.in/submitsms.jsp?user=Rsgroup&key=950436fdcaXX&mobile=+91".$mobile."&message=Dear Investor We got your investment plan request Our team will verify your account and after verification will get a notification from our team RSGROUP&senderid=RSGRUP&accusage=1";
...
$dup=array('activate'=>1);
$this->db->where('id',$id);
$this->db->update('investors',$dup);
...
public function addpage_app()
    ...
    public function allpageapp1()
    {
       if(_is_user_login($this)){
           ...
           $this->load->view('admin/page_app/editpage_app1',$data);
        }
        else
        {
            redirect('login');
