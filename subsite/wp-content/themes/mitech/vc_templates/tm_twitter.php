...
if ( in_array( $style, array( 'slider-quote', 'slider-quote-light' ), true ) ) {
	$css_class .= ' group-style-01';
}

$is_swiper = false;
if ( in_array( $style, array( 'slider', 'slider-quote', 'slider-quote-light' ), true ) ) {
	$is_swiper = true;
				...
				<div class="<?php echo esc_attr( $grid_classes ); ?>">
					...
					<?php if ( $style === 'slider' ) { ?>
						<?php foreach ( $twitter as $tweet ) : ?>
							...
							$latest_tweet = preg_replace( '/@([a-z0-9_]+)/i', '&nbsp;<a href="https://twitter.com/$1" target="_blank">@$1</a>&nbsp;', $latest_tweet );
							?>
						...
						<?php endforeach; ?>
					<?php } elseif ( in_array( $style, array( 'slider-quote', 'slider-quote-light' ), true ) ) { ?>
						<?php foreach ( $twitter as $tweet ) : ?>
							<?php
							$latest_tweet = $tweet['text'];
							$latest_tweet = preg_replace( '/(https:\/\/[a-z0-9\.\/]+)/i', '&nbsp;<a href="$1" target="_blank">$1</a>&nbsp;', $latest_tweet );
