...
if ( ! class_exists( 'Mitech_VC_Number_Responsive' ) ) {

	class Mitech_VC_Number_Responsive {
		...
		public function render() {
			...
			$sizes = array(
				'lg' => esc_html__( 'Large Device', 'mitech' ),
				'md' => esc_html__( 'Medium Device', 'mitech' ),
				'sm' => esc_html__( 'Small Device', 'mitech' ),
				'xs' => esc_html__( 'Extra Small Device', 'mitech' ),
			);
		...
		public function media_query_input( $icon, $tooltip, $key, $min, $max, $step ) {
			...
			$html .= '<label for="' . $key . '" class="hint--top-right hint-bounce" data-hint="' . $tooltip . '">' . $icon . "</label>";
			$html .= '<input id="' . $key . '" type="number" min="' . $min . '" max="' . $max . '" step="' . $step . '" class="' . $this->settings['type'] . '_field" value="' . $this->get_number( $key ) . '"/>';
			$html .= '<div class="buttons">';
			$html .= '<input type="button" value="+" class="plus" tabindex="-1" />';
			$html .= '<input type="button" value="-" class="minus" tabindex="-1" />';
			$html .= '</div>';
			$html .= '</div>';
...
if ( class_exists( 'Mitech_VC_Number_Responsive' ) ) {
	...
	function mitech_number_responsive_settings_field( $settings, $value ) {

		$number_responsive = new Mitech_VC_Number_Responsive( $settings, $value );

		return $number_responsive->render();
	}

	WpbakeryShortcodeParams::addField( 'number_responsive', 'mitech_number_responsive_settings_field', MITECH_THEME_URI . '/vc-extend/vc-params/number_responsive/number_responsive.js' );
}
